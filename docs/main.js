...
fetch("https://backend-gps-zenodo.onrender.com/windy-key")
  .then(res => res.json())
  .then(data => {
    windyKey = data.key;
    initMapWithWindy();
  ...
  .catch(err => {
    console.error("❌ No se pudo obtener la API key de Windy:", err);
  });

function initMapWithWindy() {
  if (typeof windyInit === 'undefined') {
    const script = document.createElement("script");
    script.src = "https://unpkg.com/windy-api@latest/windy.js";
    script.onload = () => {
      windyInit({ key: windyKey, verbose: false }, windyAPI => {
        const { map: windyMap } = windyAPI;
        map = windyMap;
        setupMainLogic(); // sigue con tu código aquí
      });
    };
